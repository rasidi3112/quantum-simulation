"""
Discretized spatial and momentum grids for quantum simulation.
"""
import numpy as np

class Grid1D:
    """
    1D Spatial and Fourier (momentum) space grid.
    """
...
class Grid2D:
    """
    2D Spatial and Fourier (momentum) space grid.
    """
    def __init__(self, N_x: int, x_min: float, x_max: float, N_y: int, y_min: float, y_max: float):
        ...
        # 2D coordinate meshgrids (indexing='ij' so shape is (N_x, N_y))
