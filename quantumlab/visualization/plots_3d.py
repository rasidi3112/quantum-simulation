...
def plot_space_time_3d(grid, t_arr: np.ndarray, space_time_data: np.ndarray, x_range: tuple=None, title: str='Quantum Wave Packet Space-Time Evolution', save_path: str=None, show: bool=True, theme: str='light'):
    set_style(theme)
    ...
    ax.set_zlabel('Probability Density $|\\Psi(x, t)|^2$', fontsize=10, labelpad=10)
    ax.set_title(title, fontsize=12, fontweight='bold', pad=15)
    ax.view_init(elev=32, azim=-62)
    cbar = fig.colorbar(surf, ax=ax, shrink=0.5, aspect=10, pad=0.1)
    cbar.set_label('Probability Density $|\\Psi|^2$', fontsize=9)
    plt.tight_layout()
    if save_path:
