...
def plot_wavefunction_1d(wf: WaveFunction1D, potential=None, title: str = "Wave Function State", 
                         save_path: str = None, show: bool = True, theme: str = 'light'):
    """
    Plot the probability density and real part of a 1D wavefunction.
    Optionally overlays a scaled potential function.
    """
    set_style(theme)
    ...
    ax.plot(x, prob, color=prob_color, linewidth=1.8, label=r'Probability Density $|\Psi(x)|^2$')
    ax.fill_between(x, 0, prob, color=prob_color, alpha=0.1)
    ax.plot(x, real_psi, color=real_color, linestyle='--', linewidth=0.8, alpha=0.6, label=r'Real Part $\mathrm{Re}(\Psi)$')
...
def plot_dual_space_1d(wf: WaveFunction1D, title: str = "Dual Space Analysis", 
                       save_path: str = None, show: bool = True, theme: str = 'light'):
    """
    Plot position space and momentum space probability densities side-by-side.
    """
    set_style(theme)
