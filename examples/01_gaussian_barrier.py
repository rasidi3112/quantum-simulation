"""
Example 01: 1D Gaussian wave packet scattering off a Gaussian potential barrier.
"""
import os
import numpy as np
...
def main():
    # Simulation parameters (aligned with original monolithic script)
    L = 100.0
    N = 1024
    dt = 0.04
    num_steps = 800
    ...
    # Array to log space-time probability density
    ...
    # 1. Final state wavefunction plot
    plot_wavefunction_1d(
        wf_current,
        potential=potential,
        title="Wave Function Scattering (Final State)",
        save_path="output/01_gaussian_barrier_final.png",
        show=False,
        theme="light"
    )

    # 2. Space-Time 3D plot
    ...
    plot_space_time_3d(
        ...
        space_time,
        x_range=(-40, 40),
        title="Gaussian Barrier Space-Time Evolution (3D)",
        save_path="output/01_gaussian_barrier_spacetime.png",
        show=False,
        theme="light"
    )
    ...
    plot_dual_space_1d(
        wf_current,
        title="Gaussian Barrier Scattering: Position vs Momentum Space",
        save_path="output/01_gaussian_barrier_dual.png",
        show=False,
        theme="light"
    )
