"""
Example 02: 1D Wave packet propagation in a Harmonic Oscillator potential.
Shows coherent state oscillation and demonstrates total energy conservation.
"""
import os
import numpy as np
...
def main():
    ...
    N = 512
    dt = 0.02
    num_steps = 600  # Total simulation time = 12.0 seconds

    # Harmonic oscillator parameters (omega=1.0, m=1.0)
    omega = 1.0
    m = 1.0

    # Initialize displaced Gaussian packet (coherent state width is sigma = 1 / sqrt(2))
    x0 = -6.0
    k0 = 0.0
    ...
    plot_wavefunction_1d(
        wf_current,
        potential=potential,
        title="Harmonic Oscillator Coherent State (Final State)",
        save_path="output/02_harmonic_oscillator_final.png",
        show=False,
        theme="light"
    )

    # 2. 3D space-time plot showing sinusoidal motion
    ...
    plot_space_time_3d(
        ...
        space_time,
        x_range=(-15, 15),
        title="Quantum Harmonic Oscillator Space-Time Evolution (3D)",
        save_path="output/02_harmonic_oscillator_spacetime.png",
        show=False,
        theme="light"
    )

    # Report conservation of energy (unitary evolution property of split-step solver)
